...
export default async function CompanyPublicPage({
  ...
  const socialLinks = [
    ...
    { href: company.linkedin_url, label: "LinkedIn", icon: "linkedin" },
    { href: company.github_url,   label: "GitHub",   icon: "github"   },
    { href: company.youtube_url,  label: "YouTube",  icon: "youtube"  },
  ].filter((s) => !!s.href);
  ...
  function SocialIcon({ icon }: { icon: string }) {
    if (icon === "globe") return (
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
        <path d="M6 1a5 5 0 100 10A5 5 0 006 1zm0 0c1.5 2 1.5 8 0 10M1 6h10" stroke="#0891b2" strokeWidth="1.1" strokeLinecap="round"/>
      </svg>
    );
    if (icon === "twitter") return (
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
        <path d="M11 1.5L6.8 6.3M11 1.5H8M11 1.5V5M1 10.5l4.2-4.8m0 0L1 1.5h3.5l6.5 9H7.5L5.2 5.7z" stroke="#0891b2" strokeWidth="1.1" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    );
    if (icon === "linkedin") return (
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
        <path d="M2 1h8a1 1 0 011 1v8a1 1 0 01-1 1H2a1 1 0 01-1-1V2a1 1 0 011-1zm4 8V5m0 0V3m0 2H4m2 0h2" stroke="#0891b2" strokeWidth="1.1" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    );
    if (icon === "github") return (
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
        <path d="M6 1a5 5 0 00-1.58 9.74c.25.05.34-.11.34-.24v-.85C3.16 9.9 2.87 9 2.87 9c-.23-.58-.56-.74-.56-.74-.46-.31.03-.3.03-.3.5.03.77.52.77.52.45.77 1.18.55 1.47.42.05-.33.18-.55.32-.68C3.43 8.1 2.37 7.7 2.37 5.9c0-.52.19-.95.5-1.28-.05-.12-.22-.61.05-1.27 0 0 .4-.13 1.32.49a4.6 4.6 0 012.4 0c.91-.62 1.31-.49 1.31-.49.27.66.1 1.15.05 1.27.31.33.5.76.5 1.28 0 1.81-1.1 2.2-2.15 2.32.17.15.32.44.32.89v1.31c0 .13.09.29.34.24A5 5 0 006 1z" stroke="#0891b2" strokeWidth="0.8"/>
      </svg>
    );
    if (icon === "youtube") return (
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
        <rect x="1" y="2.5" width="10" height="7" rx="1.5" stroke="#0891b2" strokeWidth="1.1"/>
        <path d="M5 4.5l3 1.5-3 1.5V4.5z" fill="#0891b2"/>
      </svg>
    );
    return null;
  }
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {socialLinks.map((s, i) => (
                <a key={i} href={s.href!} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ background: "#f0f9ff", border: "1px solid #bae6fd", color: "#0891b2", textDecoration: "none" }}>
