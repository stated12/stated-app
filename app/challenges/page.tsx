...
export const metadata: Metadata = {
  ...
  description: "Post a challenge. Receive real work as applications. Find exactly who you need with execution as the filter — not credentials.",
  metadataBase: new URL("https://app.stated.in"),
  openGraph: {
    title: "Stated Challenges — Hire Based on Work, Not CVs",
    description: "Post a challenge. Receive real work. Find the best executor.",
...
export default async function ChallengesPage({
  ...
  searchParams: { type?: string; q?: string; view?: string };
}) {
  ...
  const view        = searchParams.view || "browse";

  let query = supabase
    .from("challenges")
    .select("id, title, type, description, what_winner_gets, deadline, expires_at, submission_count, view_count, featured, plan, location, tags, posted_by_type, posted_by_user_id, company_id")
    .eq("status", "active")
  ...
  return (
    <div className="min-h-screen bg-white" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ── HERO ──────────────────────────────────────────────────────── */}
      <section style={{ background: "linear-gradient(180deg,#eef5ff 0%,#ffffff 100%)" }} className="px-6 pt-16 pb-0 border-b border-gray-100">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <div className="flex flex-wrap gap-3 justify-center mb-10">
              <Link href="/challenges/new" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-7 py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5">
                Post a Challenge
              </Link>
      ...
      <section className="bg-white px-6 py-16 border-b border-gray-100">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-extrabold text-gray-900 mb-3">What can you post?</h2>
            <p className="text-gray-500 text-base max-w-xl mx-auto">Every challenge type is designed for a specific outcome. Pick what fits your need.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {(Object.entries(CHALLENGE_TYPES) as [ChallengeType, any][]).map(([key, cfg]) => {
              ...
              return (
                <Link key={key} href={`/challenges?type=${key}`} className={`block border ${ac.border} rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all no-underline bg-white`}>
                  ...
                  <div className="p-5">
                    ...
                    <div className="text-xs text-gray-400 mb-3 leading-relaxed">{cfg.for}</div>
                    <div className="text-xs text-gray-500 leading-relaxed mb-4">{cfg.description}</div>
      ...
      <section className="bg-gray-50 px-6 py-16 border-b border-gray-100">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-extrabold text-gray-900 mb-3">Built for real hiring and partnerships</h2>
            <p className="text-gray-500 text-base max-w-xl mx-auto">Everything you need to find and evaluate the right people — without the noise.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              { icon: "🎯", title: "Work-based applications",    body: "Applicants submit real work — not just a CV. You evaluate execution, not credentials. The best outcome wins." },
              { icon: "📊", title: "Submissions dashboard",      body: "Review every application in one place. Shortlist, reject, and mark winners. Notes and status tracking built in." },
              { icon: "📧", title: "Email invites included",     body: "Send direct invites to your contact list. Import via CSV or paste emails manually. All plans include invite credits." },
              { icon: "🌐", title: "Public by default",          body: "Every challenge gets a public page with a shareable URL. Share on LinkedIn, WhatsApp, Twitter to maximise reach." },
              { icon: "⭐", title: "Featured placement",         body: "Pro and Scale plans get featured on the Stated challenges browse page. More visibility, more quality applicants." },
              { icon: "🔔", title: "Instant notifications",     body: "Email notification the moment someone submits. Submitter gets a confirmation. You never miss an application." },
            ].map(({ icon, title, body }) => (
      ...
      <section className="bg-white px-6 py-16 border-b border-gray-100">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-extrabold text-gray-900 mb-3">One payment. Challenge goes live.</h2>
            <p className="text-gray-500 text-base max-w-lg mx-auto">Prices vary by challenge type. These are for a Hiring Challenge — pick your type to see exact pricing.</p>
          </div>
            ...
            {(["basic","pro","scale"] as const).map(p => {
              ...
              return (
                <div key={p} className={`border-2 rounded-2xl p-7 relative ${p === "pro" ? "border-blue-600 bg-blue-50" : "border-gray-200 bg-white"}`}>
                  ...
                  <div className={`text-3xl font-extrabold font-display mb-1 ${p === "pro" ? "text-blue-700" : "text-gray-900"}`}>
                  ...
                  <div className="flex flex-col gap-3 mb-6">
                    {[
                      ...
                      { label: "Max submissions",   val: pl.maxSubmissions >= 999999 ? "Unlimited" : pl.maxSubmissions.toString() },
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {[
                { contacts: 25,  price: 149  },
                { contacts: 50,  price: 249  },
                { contacts: 100, price: 449  },
                { contacts: 250, price: 899  },
                { contacts: 500, price: 1599 },
              ].map(({ contacts, price }) => (
                <div key={contacts} className="bg-white border border-gray-200 rounded-xl p-3 text-center">
                  <div className="text-lg font-bold text-gray-900">{contacts}</div>
                  <div className="text-xs text-gray-400 mb-1">contacts</div>
                  <div className="text-sm font-bold text-blue-600">Rs.{price}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-14 items-center">
            <div>
              ...
              <h2 className="font-display text-3xl font-extrabold text-white mb-5 leading-tight">
                Reach exactly the right people.<br />
                <span className="text-blue-400">From inside Stated.</span>
              </h2>
              <p className="text-gray-400 text-base leading-relaxed mb-6">
                Import your contact list as a CSV or paste emails manually. Stated sends personalised
                challenge invite emails on your behalf — with your challenge title, deadline, and a
                direct link to apply. All tracked. All in your dashboard.
              </p>
              <div className="flex flex-col gap-3">
                {[
                  { icon: "📁", label: "CSV import",          desc: "Upload a file with name, email, company — Pro and Scale plans" },
                  { icon: "✉️",  label: "Manual email entry",  desc: "Paste emails one by one — works on all plans" },
                  { icon: "📬", label: "Branded invite email", desc: "Sent from Stated with your challenge details — professional and direct" },
                  { icon: "📊", label: "Delivery tracking",    desc: "See who was invited, sent, and how many invites remain" },
                ].map(({ icon, label, desc }) => (
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              ...
              <div className="bg-white rounded-xl p-5">
                ...
                <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 mb-3">
                  ...
                  <div className="text-xs text-gray-500 mt-1">Posted by Credorium Labs · Deadline: July 15</div>
                </div>
      ...
      <section id="browse" className="bg-white px-6 py-16 border-t border-gray-100">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 mb-6">
            ...
            <div className="flex gap-3">
              <form method="GET" action="/challenges" className="flex bg-white border border-gray-300 rounded-xl overflow-hidden shadow-sm">
                <input type="text" name="q" defaultValue={searchQuery} placeholder="Search challenges..." className="px-4 py-2.5 text-gray-900 text-sm outline-none placeholder-gray-400 bg-transparent w-48" />
                {filterType !== "all" && <input type="hidden" name="type" value={filterType} />}
                <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-4 transition-colors">Go</button>
              </form>
          ...
          {/* Filter tabs */}
          <div className="flex gap-0 overflow-x-auto border-b border-gray-200 mb-8 scrollbar-hide">
            {TYPE_FILTERS.map(f => (
              <Link
                key={f.value}
                href={f.value === "all" ? "/challenges#browse" : `/challenges?type=${f.value}#browse`}
                className={`whitespace-nowrap text-xs font-semibold px-4 py-3 border-b-2 transition-all no-underline ${filterType === f.value ? "border-blue-600 text-blue-600" : "border-transparent text-gray-400 hover:text-gray-700"}`}
              >
                {f.label}
              </Link>
            ))}
          ...
          {!challenges?.length ? (
            <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-400 text-sm mb-6">
                {searchQuery ? `No results for "${searchQuery}"` : "Be the first to post a challenge in this category."}
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {challenges.map((c: any) => {
                ...
                const posterName = c.posted_by_type === "company" ? "Company" : "Individual";
                  ...
                  <Link key={c.id} href={`/challenges/${c.id}`} className={`bg-white border ${ac.border} rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all no-underline`}>
                    ...
                    <div className="p-5">
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        ...
                        {c.featured && (
                          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-amber-50 text-amber-700">Featured</span>
                        )}
                        <span className="ml-auto text-xs text-gray-400">{daysLeft(c.expires_at)}</span>
                      </div>
      ...
      <section className="bg-gray-900 px-6 py-20">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-gray-400 text-base font-light leading-relaxed mb-8 max-w-lg mx-auto">
            ...
            Start evaluating people who show you what they can do.
          </p>
